...
- `MouseTrailForm.cs`：全虚拟桌面的透明分层覆盖窗，负责窗口生命周期、鼠标穿透、DIB/HDC 缓冲和画面提交；启动后预热并保持透明显示，空闲时只清空内容，避免每笔手势首帧重新显示窗口。新轨迹接管旧提示时会合并两者脏矩形后一次清除、重绘和提交，提示和 OSD 淡出帧也只重绘自身区域；命中手势结束时同样按旧轨迹与提示范围处理，避免重复更新整个虚拟桌面。即使轨迹线关闭，提示仍可单独使用此覆盖层显示。
