...
- `程序` 页右侧只展示手势列表；程序页左侧只列出已经创建 app 规则作用域的程序，不再把分类页关联的应用程序自动混入程序规则列表。
- 分类新增通过名称弹窗完成，不再使用左侧内联输入框。
...
- 规则表列为 `名称`、`手势`、`命令`，删除按钮在每行右侧；双击规则行、点击手势列或点击命令列都会打开规则编辑弹窗，手势列用 `◑` 表示右键、`●` 表示中键，并追加 8 方向箭头。
