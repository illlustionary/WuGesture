...
export function normalizeMouseTrailSettings(settings) {
  ...
  return {
    ...
    inactiveThickness: clampFloat(
      ...
      DEFAULT_UI_SETTINGS.mouseTrail.inactiveThickness
    ),
    activeThickness: clampFloat(
      settings?.activeThickness,
      GESTURE_EDITOR_LIMITS.mouseTrailThickness.min,
      GESTURE_EDITOR_LIMITS.mouseTrailThickness.max,
      DEFAULT_UI_SETTINGS.mouseTrail.activeThickness
    ),
    inactiveOpacity: clampInteger(
      settings?.inactiveOpacity,
      GESTURE_EDITOR_LIMITS.opacityPercent.min,
      GESTURE_EDITOR_LIMITS.opacityPercent.max,
      DEFAULT_UI_SETTINGS.mouseTrail.inactiveOpacity
    ),
    activeOpacity: clampInteger(
      settings?.activeOpacity,
      GESTURE_EDITOR_LIMITS.opacityPercent.min,
      GESTURE_EDITOR_LIMITS.opacityPercent.max,
      DEFAULT_UI_SETTINGS.mouseTrail.activeOpacity
    )
  }
}
...
export function normalizeGestureHintSettings(settings) {
  ...
  return {
    ...
    fontFamily:
      String(settings?.fontFamily ?? DEFAULT_UI_SETTINGS.gestureHint.fontFamily).trim() ||
      DEFAULT_UI_SETTINGS.gestureHint.fontFamily,
    fontSize: clampFloat(
      settings?.fontSize,
      GESTURE_EDITOR_LIMITS.hintFontSize.min,
      GESTURE_EDITOR_LIMITS.hintFontSize.max,
      DEFAULT_UI_SETTINGS.gestureHint.fontSize
    ),
    textColor:
      String(settings?.textColor ?? DEFAULT_UI_SETTINGS.gestureHint.textColor).trim() ||
      DEFAULT_UI_SETTINGS.gestureHint.textColor,
    backgroundColor:
      String(settings?.backgroundColor ?? DEFAULT_UI_SETTINGS.gestureHint.backgroundColor).trim() ||
      DEFAULT_UI_SETTINGS.gestureHint.backgroundColor,
    backgroundOpacity: clampInteger(
      settings?.backgroundOpacity,
      GESTURE_EDITOR_LIMITS.opacityPercent.min,
      GESTURE_EDITOR_LIMITS.opacityPercent.max,
      DEFAULT_UI_SETTINGS.gestureHint.backgroundOpacity
    ),
    widthPercent: clampInteger(
      ...
      DEFAULT_UI_SETTINGS.gestureHint.widthPercent
    ),
    ...
    heightPercent: clampInteger(
      ...
      DEFAULT_UI_SETTINGS.gestureHint.heightPercent
    ),
    cornerRadius: clampFloat(
      settings?.cornerRadius,
      GESTURE_EDITOR_LIMITS.hintCornerRadius.min,
      GESTURE_EDITOR_LIMITS.hintCornerRadius.max,
      DEFAULT_UI_SETTINGS.gestureHint.cornerRadius
    ),
    bottomOffsetPercent: clampInteger(
      settings?.bottomOffsetPercent,
      GESTURE_EDITOR_LIMITS.hintBottomOffsetPercent.min,
      GESTURE_EDITOR_LIMITS.hintBottomOffsetPercent.max,
      DEFAULT_UI_SETTINGS.gestureHint.bottomOffsetPercent
    )
  }
}
...
function normalizeLevelOsdPosition(value) {
  const normalized = String(value ?? '').trim()
  return ['center', 'top-center', 'bottom-center', 'top-left', 'top-right', 'bottom-left', 'bottom-right'].includes(
    normalized
  )
    ? normalized
...
export function normalizeAppBehaviorSettings(settings) {
  ...
  return {
    ...
    disableGesturesInFullscreen: Boolean(
      settings?.disableGesturesInFullscreen ?? DEFAULT_UI_SETTINGS.appBehavior.disableGesturesInFullscreen
    ),
    disableEdgeActionsInFullscreen: Boolean(
      settings?.disableEdgeActionsInFullscreen ?? DEFAULT_UI_SETTINGS.appBehavior.disableEdgeActionsInFullscreen
    ),
...
export function normalizeAppearanceSettings(settings) {
  ...
  return {
    theme: ['system', 'light', 'dark'].includes(theme) ? theme : DEFAULT_UI_SETTINGS.appearance.theme,
    lightTitleBarColor: normalizeColor(settings?.lightTitleBarColor, DEFAULT_UI_SETTINGS.appearance.lightTitleBarColor),
    ...
    darkTitleBarColor: normalizeColor(settings?.darkTitleBarColor, DEFAULT_UI_SETTINGS.appearance.darkTitleBarColor),
...
export function normalizeMouseButton(button) {
  return String(button ?? '').toLowerCase() === MOUSE_BUTTONS.middle ? MOUSE_BUTTONS.middle : MOUSE_BUTTONS.right
}
...
export function normalizeWindowOperation(operation) {
  ...
  return WINDOW_OPERATIONS.some(item => item.value === normalized) ? normalized : OPERATIONS.toggleMaximize
}

export function normalizeVolumeOperation(operation) {
  const normalized = String(operation ?? '').trim()
  return VOLUME_OPERATIONS.some(item => item.value === normalized) ? normalized : OPERATIONS.increase
}

export function normalizeBrightnessOperation(operation) {
  const normalized = String(operation ?? '').trim()
  return BRIGHTNESS_OPERATIONS.some(item => item.value === normalized) ? normalized : OPERATIONS.increase
}

export function normalizeAmount(value) {
  return clampInteger(
    value,
    GESTURE_EDITOR_LIMITS.amount.min,
    GESTURE_EDITOR_LIMITS.amount.max,
    GESTURE_EDITOR_LIMITS.amount.fallback
  )
}
...
export function normalizeFrictionCount(value) {
  return clampInteger(
    value,
    GESTURE_EDITOR_LIMITS.frictionCount.min,
    GESTURE_EDITOR_LIMITS.frictionCount.max,
    GESTURE_EDITOR_LIMITS.frictionCount.fallback
  )
}
...
export function normalizeEdgeLocation(location, triggerType) {
  ...
  return locations.some(item => item.value === normalized) ? normalized : locations[0].value
}

export function normalizeWheelDirection(direction) {
  return String(direction ?? '').toLowerCase() === WHEEL_DIRECTIONS.down ? WHEEL_DIRECTIONS.down : WHEEL_DIRECTIONS.up
}
